function validateKeys({ fieldsKeys, dataKeys }) {
  ...
  for (let k = 0; k < fieldsKeys.length; k++) {
    ALL_KEYS.push(utils.findStringInArray(fieldsKeys, dataKeys[k]));
  }
  const COMPARE_KEYS = utils.findMultipleIndex(ALL_KEYS, false);
  if (COMPARE_KEYS.length) {
    for (let key = 0; key < COMPARE_KEYS.length; key++) {
      if (utils.isUndefined(dataKeys[COMPARE_KEYS[key]])) {
        throw new Error(
