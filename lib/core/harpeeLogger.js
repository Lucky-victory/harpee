...
HarpeeLogger.prototype.readLog= async function(options,callback) {
  let res;
...
const past3YearsInISOString=new Date(past3YearsInMilliseconds).toISOString();
...
HarpeeLogger.prototype.getJob= async function(jobId,callback) {
  let res;
 const id=jobId;
 if(!id){
...
HarpeeLogger.prototype.readTransactionLog= async function(options,callback) {
  let res;
...
HarpeeLogger.prototype.readTransactionLogByTimestamp= async function(options,callback) {
  let res;
  const schema = options.schema || this.modelSchemaConfig.schemaObject.name;
  const table = options.table || this.modelSchemaConfig.modelName;
...
HarpeeLogger.prototype.readTransactionLogByUser= async function(options,callback) {
  let res;
  const schema = options.schema || this.modelSchemaConfig.schemaObject.name;
  const table = options.table || this.modelSchemaConfig.modelName;
  const search_values = options.searchValues;
...
HarpeeLogger.prototype.deleteRecordsBefore= async function(options,callback) {
  let res;
  const schema=options.schema || this.modelSchemaConfig.schemaObject.name;
  const table=options.table || this.modelSchemaConfig.modelName;
...
HarpeeLogger.prototype.deleteTransactionLogsBefore= async function(options,callback) {
  let res;
  const schema=options.schema || this.modelSchemaConfig.schemaObject.name;
  const table=options.table || this.modelSchemaConfig.modelName;
...
HarpeeLogger.prototype.searchJobsByStartDate= async function(options,callback) {
  let res;
  if (!Util.isObject(options)) {
     throw new Error('`options` must be an object');
  }
