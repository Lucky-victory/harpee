...
export class SqlHandler {
  ...
  offset(offset: number) {
    this._query += Utils.notNullOrUndefined(offset) ? ` OFFSET ${offset}` : '';
    return this;
  }
  /**
   * @param offset - the number of rows to skip.
  ...
  in(values: StringOrNumber[]) {
    this._query += Utils.isArray(values) ? ` IN ("${values.join('","')}")` : '';
    return this;
  }
  ...
  crossJoin(schema: string, table: string) {
    this._query += schema && table ? ` CROSS JOIN ${schema}.\`${table}\`` : '';
    return this;
  }
  ...
  innerJoin(schema: string, table: string) {
    this._query += schema && table ? ` INNER JOIN ${schema}.\`${table}\`` : '';
    return this;
  }
