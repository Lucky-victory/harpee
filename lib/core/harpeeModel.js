...
const callbackOrPromise = require("./harpeeHttpAlt");
 ...
 * @param {*} result - response result or null if an error occurs
 */
...
class HarpeeModel extends HarpeeHttp {
   ...
   constructor(modelName, schemaObject) {
      ...
      const config = harpeeConnect.getConfig();
      (async function() {
         try {
         const checker = {};
     const {data}= await callbackOrPromise(config,{
               operation: operations.DESCRIBE_ALL
            });
checker['schema']=data[SCHEMA_NAME];
if(!checker.schema){
   await callbackOrPromise(config, {
...
console.log(err)
    ...
    * @example
    * // with callback
    * myUsers.query('SELECT * FROM myUsersSchema.users',(err,result)=>{
      if(err) console.log(err);
      console.log(result)
    });
    @example
    * // expecting a promise
    *  myUsers.query('SELECT * FROM myUsersSchema.users').then((result)=> console.log(result)).catch((err)=> console.log(err));
    */
    ...
    *  myUsers.describeModel((err,result)=>{
     ...
     @example
    * // expecting a promise
    * myUsers.describeModel().then((result)=> console.log(result)).catch((err)=>console.log(err));
    */
   ...
   /** 
    * Returns multiple result from the table.
    ...
    * myUsers.find([],(err,result)=>{
        if(err) console.log(err);
        console.log(result)
     });
    * @example
    * // expecting a promise
       myUsers.find(['age','user_id']).then((result)=> console.log(result)).catch((err)=> console.log(err));
    *   @example 
    * // using an object parameter
    * 
    * myUsers.find({limit:10,offset:5,orderby:'age',order:'desc',getAttributes:['age','user_id','city']},(err,result)=>{
   ...
   /**
    * Returns a single result from the table matching the specified id or value.
    * @param {(Array<string> | Object)} id - an array of a string id or an object.
    * @param {Array<string>} [id.getAttributes] - when an object is provided,you can specify which tabke column to return. 
    * @param {responseCallback} [callback] - optional `callback` function, if not provided `Promise` is returned.
    * @example
    *  // when using an array of string, it must be a value of the `primaryKey` specified in your {@link Schema}, e.g user_id.
    * myUsers.findById(['user-34567'],(err,result)=>{
        if(err) console.log(err);
        console.log(result);
     });
    * @example
    * // with object parameter
    * myUsers.findById({user_id:'user-3400',getAttributes:['age']}).then((result)=> console.log(result)).catch((err)=>console.log(err));
    */
   ...
   /**
    * Returns a single result from table matching the specified value.
    * 
    * @param {Object} attrObj
    * @param {responseCallback} [callback] - optional `callback` function, if not provided `Promise` is returned.
    * @example
    * 
    *  myUsers.findOne({age:25}).then((result)=> console.log(result)).catch((err)=>console.log(err));
    */
   ...
   /** 
    * Deletes result from the table, matching the specified id.
    *
    ...
    * 
    * myUsers.findByIdAndRemove('user-3400',(err,result)=>{
        if(err) console.log(err);
        console.log(result)
    });
    */
   ...
   /**
    * Deletes multiple result from table based on the specified values.
    *
    * @param {Object} attrObj
    *@param {responseCallback} [callback] - optional `callback` function, if not provided `Promise` is returned.
    * @example
    * // this will delete users with age 25 and 30.
    * myUsers.findAndRemove({age:[25,30]},(err, result) => {
    *  if (err) console.log(err);
    *             console.log(result)
    * });
    */
    ...
    * 
    * myUsers.update([{
       user_id:'user-3400',age:23,name:'Stevie' }],(err,result)=>{
        console.log(result) 
   ...
   /**
    * Inserts new result to the table,
    * 
   ...
   /**
    * Load result to a table from a CSV string, alias for {@link https://api.harperdb.io/#92ad4d1d-5feb-492e-96d1-7203c7df6ad6 | csv_result_load operation}.
    * @param {Object} options
    * @param {string} options.csv - a valid CSV string.
    * @param {string} [options.action='insert'] - what action to be performed on the result - 'insert','update' or 'upsert'. default is 'insert'.
    * @param {responseCallback} [callback] - optional `callback` function, if not provided `Promise` is returned.
    */
   async importFromCsv(options, callback) {
      try {
         ...
         res = await this.$callbackOrPromise(
            {
               operation: operations.CSV_result_LOAD,
               action,
               schema: this.SCHEMA_NAME,
               table: this.MODEL_NAME,
               result,
   ...
   /**
    * Load result to a table from a local CSV file, alias for {@link https://api.harperdb.io/#e74330f8-7f61-46cf-9b52-f27962d73c05 | csv_file_load operation} .
    * @param {Object} options
   ...
   /**
    * Load result to a table from an external  CSV file, alias for {@link https://api.harperdb.io/#309d4440-c40d-497c-a30a-a3f0d4024fbc | csv_url_load operation}.
    * @param {Object} options
   ...
   /**
    * Load result to a table from amazon S3, visit {@link https://api.harperdb.io/#d301d85f-826a-4bda-af5d-a5fb0a04bde2 | import_from_s3 operation} to learn more.
    * @param {Object} options
   ...
   /**
    * Deletes every result from the table, **use this with caution**;
    *
   ...
   /**
    * Retruns result from table using matching conditions, alias for {@link https://api.harperdb.io/#c820c353-e7f6-4280-aa82-83be77857653 | search_by_conditions operation}.
    * @param {Object} options
    * @param {number} [options.limit=null] - a limit of result to be returned, default is `null`.
    * @param {number} [options.offset=0] - number of result to be skipped, default is `0`.
   ...
   /**
    *  Returns result from a table with matching values, alias for {@link https://api.harperdb.io/#647beb0d-f26d-473e-84bb-9805dfca3835 | search_by_value operation}.
    * @param {Object} options
    * @param {string[]} [options.get_attributes] - an array of one or more attributes to be returned, default is `["*"]` which returns all attributes.
