...
async init(){
      ...
      const createSchema = this.$callbackOrPromise({
            operation: operations.CREATE_SCHEMA,
            schema: this.schemaName
         });
      
      const describeAll = this.$callbackOrPromise({
            operation: operations.DESCRIBE_ALL,

         });
      
      const createTable = this.$callbackOrPromise({
               ...
               if (!(respB[schemaName] && respB[schemaName][modelName])) {
                  await createTable()
               }
