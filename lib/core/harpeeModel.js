...
class HarpeeModel extends HarpeeHttp {
     ...
     * @param {array} [options.get_attributes] - an array of attributes to retrieve, default is `['*']`.
     * @param {responseCallback} [callback] - optional `callback` function, if not provided `Promise` is returned.
     * @example
     ...
     * 
     * myUsers.find({limit:10,offset:5,orderby:'age',order:'desc',get_attributes:['age','user_id','city']},(err,data)=>{
     ...
     * @param {responseCallback} [callback] - optional `callback` function, if not provided `Promise` is returned.
     * @example
     *  // when using string, it must be a value of the `primaryKey` specified in your `Schema`, e.g user_id.
