...
Model.prototype.find = async function(options, callback) {
  if (!U.isObject(options)) {
    throw new TypeError(' find `options` must be an object')
  }
  // 
  // {
  //   limit?:number,
...
Model.prototype.create = async function(newData, callback) {
  let res;

  if (!U.isObject(newData)) {
    throw new TypeError(" `newData` must be an object");
  }
  validator(this.SCHEMA_FIELDS, newData);
  try {
